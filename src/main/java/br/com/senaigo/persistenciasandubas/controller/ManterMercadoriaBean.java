...
@CrossOrigin(origins = "*")
public class ManterMercadoriaBean {

	@Autowired
    private MercadoriaService service;

	@GetMapping
    public ResponseEntity<Response<List<Mercadoria>>> findAll() {
        return ResponseEntity.ok(RestControllerUtil.findAll(service));
    }
 
    @GetMapping(value = "{id}")
    public ResponseEntity<Response<Mercadoria>> findById(@PathVariable("id") String id) {
    	return ResponseEntity.ok(RestControllerUtil.findById(service, id));
    }
 
    @PostMapping
    public ResponseEntity<Response<Mercadoria>> newObject(@RequestBody Mercadoria objeto) {
    	return ResponseEntity.ok(RestControllerUtil.save(service, objeto));
    }
 
    @PutMapping
    public ResponseEntity<Response<Mercadoria>> update(@RequestBody Mercadoria objeto) {
    	return ResponseEntity.ok(RestControllerUtil.save(service, objeto));
    }
    ...
    public ResponseEntity<Response<Mercadoria>> findByField(@PathVariable("field") String field, 
