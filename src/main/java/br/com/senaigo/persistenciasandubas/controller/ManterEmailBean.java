...
@CrossOrigin(origins = "*")
public class ManterEmailBean {

	@Autowired
    private EmailService service;

	@GetMapping
    public ResponseEntity<Response<List<Email>>> findAll() {
        return ResponseEntity.ok(RestControllerUtil.findAll(service));
    }
 
    @GetMapping(value = "{id}")
    public ResponseEntity<Response<Email>> findById(@PathVariable("id") String id) {
    	return ResponseEntity.ok(RestControllerUtil.findById(service, id));
    }
 
    @PostMapping
    public ResponseEntity<Response<Email>> newObject(@RequestBody Email objeto) {
    	return ResponseEntity.ok(RestControllerUtil.save(service, objeto));
    }
 
    @PutMapping
    public ResponseEntity<Response<Email>> update(@RequestBody Email objeto) {
    	return ResponseEntity.ok(RestControllerUtil.save(service, objeto));
    }
    ...
    public ResponseEntity<Response<Email>> findByField(@PathVariable("field") String field, 
